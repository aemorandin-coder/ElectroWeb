...
export default function ProductClient({ initialProduct }: { initialProduct: Product }) {
  ...
  const [digitalUsername, setDigitalUsername] = useState('');
  const [digitalUsernameError, setDigitalUsernameError] = useState('');

  // Fetch reviews and related products after product is loaded
  useEffect(() => {
    if (product?.id) {
  ...
  const fetchRelatedProducts = async (currentProduct: any) => {
    ...
    try {
      // Optimizamos enviando category y limit por query params a la API
      const response = await fetch(`/api/products/public?category=${currentProduct.category?.id || ''}&limit=12`);
      if (response.ok) {
        ...
        if (sameCategoryProducts.length >= 4) {
          ...
          // Si no hay suficientes, traemos otros productos generales
          const fallbackRes = await fetch(`/api/products/public?limit=12`);
          if (fallbackRes.ok) {
            const fallbackData = await fallbackRes.json();
            const otherProducts = fallbackData.filter(
  ...
  const handleAddToCart = () => {
    ...
    if (product.productType !== 'DIGITAL') {
      ...
      if (totalQuantity > product.stock) {
        toast.error(`Solo hay ${product.stock} unidades disponibles. Ya tienes ${currentCartQuantity} en el carrito.`);
        return;
      }
    }

    // Validation for digital username (only if product is digital and has MANUAL delivery method)
    if (product.productType === 'DIGITAL' && product.deliveryMethod === 'MANUAL') {
      if (!digitalUsername.trim()) {
        setDigitalUsernameError('El nombre de usuario o cuenta es obligatorio para recargas directas');
        const input = document.getElementById('digitalUsernameInput');
        if (input) {
          input.focus();
    ...
    // For digital products, create unique cart ID with amount and username (if manual)
    const isDigital = product.productType === 'DIGITAL' && selectedDigitalAmount;
    const isManualDigital = isDigital && product.deliveryMethod === 'MANUAL';
    const cartItemId = isDigital
      ? `${product.id}-${selectedDigitalAmount.amount}${isManualDigital && digitalUsername ? `-${digitalUsername.trim().toLowerCase()}` : ''}`
      : product.id;
    ...
    addItem({
      ...
      digitalUsername: isManualDigital ? digitalUsername.trim() : undefined,
    }, quantity);
  ...
  const getPlatformIcon = (platform: string) => {
    const icons: Record<string, { icon: React.ReactNode; name: string }> = {
      ...
      'GOOGLE_PLAY': { icon: <SiGoogleplay className="w-4 h-4" />, name: 'Google Play' },
      'FREEFIRE': { icon: <FiTarget className="w-4 h-4" />, name: 'Free Fire' },
      'VALORANT': { icon: <FiTarget className="w-4 h-4" />, name: 'Valorant' },
      'FORTNITE': { icon: <FiTarget className="w-4 h-4" />, name: 'Fortnite' },
      'PUBG': { icon: <FiTarget className="w-4 h-4" />, name: 'PUBG Mobile' },
    };
    return icons[platform] || { icon: <FiPackage className="w-4 h-4" />, name: platform };
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#f8f9fa] via-white to-[#f8f9fa] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-lg font-semibold text-[#212529]">Cargando producto...</p>
          ...
          <div className="space-y-3 lg:space-y-6">
            ...
            {product.productType === 'DIGITAL' && product.specs?.digitalPricing && (
              <div className="space-y-3">
                ...
                {/* Amount Selector - Premium Gaming Cards with Glow */}
                <div className="bg-gradient-to-br from-blue-50/50 via-white to-purple-50/30 rounded-2xl border border-blue-100/50 p-4 shadow-sm">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-transparent bg-clip-text bg-gradient-to-r from-[#2a63cd] to-purple-600 mb-3 flex items-center gap-1">
                    <svg className="w-3.5 h-3.5 text-[#2a63cd]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-2.5">
                    {(product.specs.digitalPricing as any[]).map((pricing: any) => {
                      const isSelected = selectedDigitalAmount?.amount === pricing.amount;
                      return (
                        <button
                          key={pricing.amount}
                          type="button"
                          onClick={() => setSelectedDigitalAmount(pricing)}
                          className={`relative py-3 px-2 rounded-xl font-black text-sm transition-all overflow-hidden flex flex-col items-center justify-center border-2 ${isSelected
                            ? 'bg-gradient-to-br from-[#2a63cd] to-[#1e4ba3] text-white border-transparent shadow-[0_4px_15px_rgba(42,99,205,0.4)] scale-105 animate-pulse'
                            : 'bg-white border-gray-200 text-gray-700 hover:border-[#2a63cd] hover:bg-blue-50/30'
                            }`}
                        >
                          <span className="text-base font-black">${pricing.amount}</span>
                          <span className={`text-[8px] mt-0.5 font-bold ${isSelected ? 'text-cyan-200' : 'text-gray-400'}`}>
                            USD
                          </span>
                        </button>
                      );
                ...
                {/* Username Input for Manual Recharge */}
                {product.deliveryMethod === 'MANUAL' && (
                  <div className="mt-4 p-4 bg-purple-50/50 rounded-xl border border-purple-100/50 space-y-2 animate-fadeIn">
                    <label htmlFor="digitalUsernameInput" className="block text-xs font-bold text-purple-700 flex items-center gap-1">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                      </svg>
                      Nombre de Usuario o Tag de Cuenta <span className="text-red-500">*</span>
                    </label>
                    <input
                      id="digitalUsernameInput"
                      type="text"
                      value={digitalUsername}
                      onChange={(e) => {
                        setDigitalUsername(e.target.value);
                        if (e.target.value.trim()) setDigitalUsernameError('');
                      }}
                      placeholder="Ej: JugadorPro#1234 o micuenta@gmail.com"
                      className={`w-full px-3 py-2 text-sm bg-white border rounded-lg text-gray-900 focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-[#7c3aed] transition-all placeholder:text-gray-400 ${digitalUsernameError ? 'border-red-300' : 'border-gray-200'}`}
                    />
                    {digitalUsernameError ? (
                      <p className="text-xs text-red-600 font-semibold">{digitalUsernameError}</p>
                    ) : (
                      <p className="text-[10px] text-purple-500 font-medium">
                        El saldo se recargará directamente a este usuario. Por favor verifica que esté bien escrito.
                      </p>
                    )}
                  </div>
                )}
              ...
              {!isOutOfStock && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="flex-[1.25] h-10 bg-gradient-to-r from-[#2a63cd] to-[#1e4ba3] text-white font-bold rounded-lg shadow-md shadow-blue-500/20 hover:shadow-blue-500/30 hover:scale-[1.01] transition-all flex items-center justify-center gap-1.5 text-xs sm:text-sm px-2.5"
                  >
                    {addingToCart ? (
                  ...
                  <button
                    ...
                    disabled={addingToCart}
                    className="flex-1 h-10 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-bold rounded-lg shadow-md shadow-emerald-500/20 hover:shadow-emerald-500/30 hover:scale-[1.01] transition-all flex items-center justify-center gap-1.5 text-xs sm:text-sm px-2.5"
                  >
