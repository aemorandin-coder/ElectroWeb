...
const ORDEN: CampoRegistro[] = ['name', 'email', 'phone', 'idNumber', 'password', 'acceptTerms'];
...
function datosDe(v: Valores) {
  return {
    ...
    idNumber: `${v.tipoDocumento}-${v.documento}`,
    password: v.password,
...
function RegistroContenido() {
  ...
  const errorDocumento = errorDe('idNumber');
    ...
    >
      ...
      <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <Campo id="phone" label="Teléfono con WhatsApp" error={errorTelefono} hint="Te escribimos por aquí sobre tus pedidos.">
          <div className="flex gap-2">
            <select
              ...
              value={valores.codigoPais}
              onChange={(e) => cambiar('codigoPais', e.target.value, 'phone')}
            ...
            <input
              ...
              onChange={(e) => cambiar('telefono', e.target.value.replace(/[^\d\s-]/g, ''), 'phone')}
              onBlur={() => tocar('phone')}
              className={adminInput(Boolean(errorTelefono))}
              placeholder={valores.codigoPais === '+58' ? '0412 123 4567' : 'Número'}
              disabled={enviando}
              {...aria('phone', true)}
            />
          </div>
        ...
        <Campo
          ...
          hint="V venezolano, E extranjero, P pasaporte, J o G RIF. Revísala: luego no se puede cambiar."
        >
          <div className="flex gap-2">
            <select
              aria-label="Tipo de documento"
              value={valores.tipoDocumento}
              onChange={(e) => cambiar('tipoDocumento', e.target.value, 'idNumber')}
              className={`${selectCorto} w-16`}
              disabled={enviando}
            >
            ...
            <input
              id="idNumber"
              type="text"
              inputMode={valores.tipoDocumento === 'P' ? 'text' : 'numeric'}
              autoComplete="off"
              ...
              onChange={(e) =>
                cambiar(
                  'documento',
                  valores.tipoDocumento === 'P' ? e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '') : e.target.value.replace(/\D/g, ''),
                  'idNumber'
                )
              }
              onBlur={() => tocar('idNumber')}
              className={adminInput(Boolean(errorDocumento))}
              placeholder={valores.tipoDocumento === 'P' ? 'Número de pasaporte' : '12345678'}
              disabled={enviando}
              {...aria('idNumber', true)}
            />
          </div>
