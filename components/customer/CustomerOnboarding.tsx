...
export default function CustomerOnboarding({ stats }: OnboardingProps) {
    ...
    return (
        <div className="bg-white rounded-2xl border border-line p-4 lg:p-5 text-gray-800 shadow-sm relative overflow-hidden mb-3">
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-4 lg:gap-6 items-center">
                ...
                <div className="flex-1 w-full space-y-2">
                    {/* Mission 1: Verify */}
                    <Link href="/customer/settings" className="group flex items-center gap-3 bg-surface hover:bg-line/60 border border-line p-2.5 rounded-xl transition-all">
                        {isEmailVerified ? (
                    ...
                    {/* Mission 2: Address */}
                    <Link href="/customer/addresses" className="group flex items-center gap-3 bg-surface hover:bg-line/60 border border-line p-2.5 rounded-xl transition-all">
                        <FiCircle className="w-4 h-4 text-brand-500 flex-shrink-0" />
                        <div className="flex-1">
                            <h3 className="text-xs font-bold text-gray-900 flex items-center gap-1.5">
                    ...
                    <Link href="/" className="group flex items-center gap-3 bg-surface hover:bg-line/60 border border-line p-2.5 rounded-xl transition-all">
                        {hasOrders ? (
                            <FiCheckCircle className="w-4 h-4 text-emerald-500 flex-shrink-0" />
                        ) : (
                            <FiCircle className="w-4 h-4 text-brand-500 flex-shrink-0" />
            ...
            <div className="relative z-10">
                ...
                >
                    <span className="flex items-center gap-2">
                        <span className={`w-1.5 h-1.5 rounded-full ${showGuide ? 'bg-brand-500' : 'bg-blue-400 animate-pulse'}`} />
                ...
                {showGuide && (
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-3 animate-fadeIn">
                        ...
                        <div className="bg-surface border border-line rounded-xl p-3 flex flex-col gap-2 hover:border-brand-500/30 transition-all duration-300">
                            <div className="w-7 h-7 rounded-lg bg-blue-50 flex items-center justify-center text-brand-500 flex-shrink-0">
