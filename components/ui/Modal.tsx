...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-[var(--z-modal)] overflow-y-auto animate-fadeIn">
      <div className="flex items-center justify-center min-h-dvh px-3 pt-4 pb-20 text-center sm:p-0">
        {/* Backdrop */}
        <div
        ...
        <div className={`relative inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:w-full ${sizes[size]} animate-scaleIn max-h-[90vh] flex flex-col`}>
          {/* Header */}
          <div className="bg-white px-4 lg:px-6 pt-4 lg:pt-6 pb-3 lg:pb-4 border-b border-line flex-shrink-0">
            <h3 className="text-base lg:text-xl font-semibold text-ink">
              {title}
            </h3>
          </div>
          ...
          {/* Footer */}
          {footer && (
            <div className="bg-surface px-4 lg:px-6 py-3 lg:py-4 flex items-center justify-end gap-2 lg:gap-3 border-t border-line flex-shrink-0">
              {footer}
            </div>
          )}
        </div>
