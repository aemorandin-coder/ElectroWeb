...
| 10d | **Masivos de productos y "Duplicar"** (encontrado en C-95) | `bulk/update` en modo Excel sin validar precio ni stock; "Precio" y "Precio %" desincronizan el "desde" de los digitales. "Duplicar" pierde las especificaciones y convierte un digital en físico (`products/page.tsx`, pantalla de ChatGPT: con permiso de Andrés mientras esté en pausa). |
...
| 13a | **C-96 · Montos exactos en la base** (encontrado en C-60b) | Prisma 6 guarda algunos `number` con arrastre binario en columnas `Decimal` (9,45 → 9.449999999999999). Pasar el dinero como texto `toFixed(2)` o `Prisma.Decimal` en órdenes, saldo, Pago Móvil, gift cards, cursos y comisiones. Comparar saldos con tolerancia de medio centavo o en centavos. Probar recarga de $9,45 + curso de $9,45. Sin migración. |
